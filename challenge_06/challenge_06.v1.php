...
function word_wrap($width, $height, $words, $font_size) {
  ...
  foreach ($words as $word) {
    if ((width($word, $font_size, $prefix_space)) > $width_left) {
      $width_left = $width - width($word, $font_size, FALSE);
      $prefix_space = FALSE;
      $new_lines ++;
    }
    else {
      $width_left = $width_left - width($word, $font_size, $prefix_space);
      $prefix_space = TRUE;
    }
  }
  //echo "new_lines: $new_lines\n";
  return ($new_lines + 1) * $font_size <= $height;
}

function width($word, $font_size, $add_space) {
  $letters = strlen($word);
  $width = $letters * $font_size;
  return $add_space ? $width + $font_size : $width;
}
